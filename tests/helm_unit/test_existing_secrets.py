...
SECRETS_DOCS_PATH = base_dir / "../nubus-docs/docs/nubus-kubernetes-operation/configuration/secrets.rst"
...
def test_existing_secrets_can_be_specified_everywhere(chart: HelmChart, subtests):
    ...
    for resource in existing_secrets_manifests:
        ...
        if kind in ("Certificate", "ConfigMap", "Ingress", "Secret", "Service", "ServiceAccount"):
            continue
        with subtests.test(kind=kind, name=name):
            failures = find_secret_usages(resource, baseline_secrets, PATH_IGNORE_LIST)
            if not failures:
                continue
            details = "\n".join(f"- '{secret}' at {section}" for secret, section in failures)
            pytest.fail(
                f"Found baseline secret name(s) in {kind}/{name}:\n{details}"
            )
